...
- Property sheet nav bar: close, title, and one "⋯" that opens a grouped menu (Switch Property… , Show as List/Grid | Views…, Rename…, Copy Address). The "+" offers Upload Files… (and Import from Google Drive… when configured) | New Folder…; it is hidden from view-only members.
